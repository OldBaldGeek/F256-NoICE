...
def assert_reset_cpu(port):
    """Assert Reset of CPU (F256 only). Suspend automatic reset/restart"""
    c256 = foenix.FoenixDebugPort()
    try:
        c256.open(port)
        c256.enter_debug()
...
def release_reset_cpu(port):
    """Release Reset of CPU (F256 only). Resume automatic reset/restart"""
    c256 = foenix.FoenixDebugPort()
    try:
        c256.open(port)
        clear_stop_indicator()
...
parser.add_argument("--start", action="store_true", dest="start",
                    help="Resume CPU processing of instructions after a STOP (F256 only).")

parser.add_argument("--assert_reset", action="store_true", dest="assert_reset",
                    help="Assert reset of the CPU, suspend auto reset (F256 only).")

parser.add_argument("--release_reset", action="store_true", dest="release_reset",
                    help="Release reset of the CPU, resume auto reset (F256 only).")

parser.add_argument("--whack", metavar="STRING", dest="whack_val",
                    help="Do debug, exit, start, stop")
...
try:
    ...
    if options.list_ports:
        ...
        elif options.assert_reset:
            assert_reset_cpu(options.port)
            print("Asserting reset...")

        elif options.release_reset:
            release_reset_cpu(options.port)
            print("Releasing reset...")

        elif options.whack_val:
            do_whack(options.port, options.whack_val)
